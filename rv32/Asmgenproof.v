...
(** Correctness proof for RV32G code generation: main proof. *)
...
(** The following lemmas show that the translation from Mach to RV32G
  preserves labels, in the sense that the following diagram commutes:
<<
                          translation
        Mach code ------------------------ RV32G instr sequence
            |                                          |
            | Mach.find_label lbl       find_label lbl |
            |                                          |
            v                                          v
        Mach code tail ------------------- RV32G instr seq tail
                          translation
>>
  The proof demands many boring lemmas showing that RV32G constructor
...
Proof.
  ...
  destruct (make_immed n); TailNoLabel.
  unfold load_hilo. destruct (Int.eq lo Int.zero); TailNoLabel.
Qed.
Hint Resolve loadimm_label: labels.

Remark addimm_label:
  forall r1 r2 n t k, tail_nolabel k (addimm r1 r2 n t k).
Proof.
...
Remark andimm_label:
  forall r1 r2 n t k, tail_nolabel k (andimm r1 r2 n t k).
Proof.
...
Remark orimm_label:
  forall r1 r2 n t k, tail_nolabel k (orimm r1 r2 n t k).
Proof.
  intros; unfold orimm.
  destruct (make_immed n); TailNoLabel.
  unfold load_hilo. destruct (Int.eq lo Int.zero); TailNoLabel.
...
Remark xorimm_label:
  forall r1 r2 n t k, tail_nolabel k (xorimm r1 r2 n t k).
Proof.
...
Proof.
  ...
  destruct (make_immed ofs); TailNoLabel.
  unfold load_hibase; TailNoLabel.
Qed.
Hint Resolve indexed_memory_access_label: labels.

Remark loadind_int_label:
  forall base ofs dst k, tail_nolabel k (loadind_int base ofs dst k).
Proof.
  intros; unfold loadind_int.
  apply indexed_memory_access_label; TailNoLabel.
Qed.
Hint Resolve loadind_int_label: labels.
...
Remark transl_cond_int_label:
  forall cond a1 a2 l k c, transl_cond_int cond a1 a2 l k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_cond_int; intros;
  destruct cond; destruct c0; TailNoLabel.
Qed.
Hint Resolve transl_cond_int_label: labels.

Remark transl_cond_float_label:
  forall cond r f1 f2 k c, transl_cond_float cond r f1 f2 k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_cond_float; intros.
  destruct cond; TailNoLabel.
Qed.
Hint Resolve transl_cond_float_label: labels.

Remark transl_cond_single_label:
  forall cond r f1 f2 k c, transl_cond_single cond r f1 f2 k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_cond_single; intros.
  destruct cond; TailNoLabel.
Qed.
Hint Resolve transl_cond_single_label: labels.

Remark transl_cond_label:
  forall cond args lbl k c, transl_cond cond args lbl k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_cond; intros; destruct cond; TailNoLabel.
  - eapply tail_nolabel_trans. eapply loadimm_label.
    eapply transl_cond_int_label; eauto.
  - eapply tail_nolabel_trans. eapply loadimm_label.
    eapply transl_cond_int_label; eauto.
  - eapply tail_nolabel_trans. eapply transl_cond_float_label; eauto.
    TailNoLabel.
  - eapply tail_nolabel_trans. eapply transl_cond_float_label; eauto.
    TailNoLabel.
  - eapply tail_nolabel_trans. eapply transl_cond_single_label; eauto.
    TailNoLabel.
  - eapply tail_nolabel_trans. eapply transl_cond_single_label; eauto.
    TailNoLabel.
Qed.

Remark transl_cond_op_int_label:
  forall cond a1 a2 d k c, transl_cond_op_int cond a1 a2 d k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_cond_op_int; intros; destruct cond; TailNoLabel.
  - destruct c0; TailNoLabel.
  - destruct c0; TailNoLabel.
Qed.

Remark transl_cond_op_label:
  forall cond args rd k c, transl_cond_op cond args rd k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_cond_op; intros; destruct cond, args; TailNoLabel.
  - eapply transl_cond_op_int_label; eauto.
  - eapply transl_cond_op_int_label; eauto.
  - eapply tail_nolabel_trans. eapply loadimm_label.
    eapply transl_cond_op_int_label; eauto.
  - eapply tail_nolabel_trans. eapply loadimm_label.
...
Remark transl_memory_access_label:
  forall (mk_instr: ireg -> int -> instruction)
         (addr: addressing) (args: list mreg) k c,
  transl_memory_access mk_instr addr args k = OK c ->
  (forall r n, nolabel (mk_instr r n)) ->
  tail_nolabel k c.
Proof.
  unfold transl_memory_access; intros; destruct addr; TailNoLabel;
  destruct (make_immed i); TailNoLabel;
  unfold load_hibase; TailNoLabel.
Qed.

Remark transl_load_label:
  forall chunk addr args dst k c, transl_load chunk addr args dst k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_load; intros; destruct chunk; TailNoLabel;
  eapply transl_memory_access_label; eauto with labels.
Qed.

Remark transl_store_label:
  forall chunk addr args src k c, transl_store chunk addr args src k = OK c -> tail_nolabel k c.
Proof.
  unfold transl_store; intros; destruct chunk; TailNoLabel;
  eapply transl_memory_access_label; eauto with labels.
Qed.
...
Proof.
  ...
  - destruct s0; TailNoLabel.
    eapply tail_nolabel_trans. eapply loadind_int_label. TailNoLabel.
    eapply tail_nolabel_trans. eapply loadind_int_label. TailNoLabel.
...
(** A valid branch in a piece of Mach code translates to a valid ``go to''
  transition in the generated RV32G code. *)
...
- The RV32G code pointed by the PC register is the translation of
  the current Mach code sequence.
- Mach register values and RV32G register values agree.
*)
...
(** We need to show that, in the simulation diagram, we cannot take
  infinitely many Mach transitions that correspond to zero transitions
  on the RV32G side.  Actually, all Mach transitions correspond to at
  least one RV32G transition, except the transition from
  [Machsem.Returnstate] to [Machsem.State].  So, the following integer
...
(* utility to distinguish the frame pointer *)
Remark preg_of_not_R30: forall r, negb (mreg_eq r R30) = true -> IR X30 <> preg_of r.
Proof.
...
- (* Mgetstack *)
  ...
  exploit loadind_correct; eauto with asmgen. discriminate.
  intros [rs' [P [Q R]]].
...
- (* Msetstack *)
  ...
  exploit storeind_correct; eauto with asmgen. discriminate.
  intros [rs' [P Q]].
  exists rs'; split. eauto.
...
(* R30 points to parent *)
  exploit loadind_correct. instantiate (1 := X30); discriminate. eexact EQ.
  instantiate (2 := rs0). rewrite DXP; eauto.
  intros [rs1 [P [Q R]]]. exists rs1; split. eauto.
  split. eapply agree_set_mreg. eapply agree_set_mreg; eauto. congruence. auto with asmgen.
  simpl; intros. rewrite R; auto with asmgen.
  apply preg_of_not_R30; auto.
(* R30 does not point to parent *)
  exploit loadind_correct. instantiate (1 := SP); discriminate. eexact EQ0.
  instantiate (2 := rs0). eauto. intros [rs1 [P [Q R]]].
  exploit loadind_correct. instantiate (1 := X30). discriminate. eexact EQ.
  instantiate (2 := rs1). simpl in Q. rewrite Q. eauto. intros [rs2 [S [T U]]].
  exists rs2; split. eapply exec_straight_trans; eauto.
  split. eapply agree_set_mreg. eapply agree_set_mreg. eauto. eauto.
  instantiate (1 := rs1#X30 <- (rs2#X30)). intros.
  rewrite Pregmap.gso; auto with asmgen.
  congruence. intros. unfold Pregmap.set. destruct (PregEq.eq r' X30). congruence. auto with asmgen.
...
- (* Mop *)
  ...
  rewrite R; auto. apply preg_of_not_R30; auto. exact I.
...
- (* Mtailcall *)
  ...
  assert (X: forall k, exists rs2,
    exec_straight tge tf
       (loadind_int SP (fn_retaddr_ofs f) RA
           (Pfreeframe (fn_stacksize f) (fn_link_ofs f) :: k)) rs0 m'0
       k rs2 m2'
    /\ rs2#SP = parent_sp s
    /\ rs2#RA = parent_ra s
    /\ forall r, if_preg r = true -> r <> SP -> r <> RA -> rs2#r = rs0#r).
  {
    intros.
    exploit loadind_int_correct. instantiate (1 := SP). discriminate.
    eexact C. intros [rs1 [P [Q R]]].
    econstructor; split.
...
+ (* Indirect call *)
  ...
  destruct (X (Pj_r x0 sig :: x)) as [rs2 [P [Q [R S]]]].
  ...
  rewrite <- (ireg_of_eq _ _ EQ1); auto with asmgen.
+ (* Direct call *)
  destruct (X (Pj_s fid sig :: x)) as [rs2 [P [Q [R S]]]].
  exploit exec_straight_steps_2. eexact P. eauto. eauto. eapply functions_transl; eauto. eauto.
  intros [ofs' [Y Z]].
  left; econstructor; split.
  eapply plus_right'. eapply exec_straight_exec; eauto.
...
- (* Mcond true *)
  ...
  inversion AT; subst. monadInv H5. left.
  generalize (functions_transl _ _ _ FIND H4); intro FN.
  generalize (transf_function_no_overflow _ _ H4); intro NOOV.
  generalize (transl_cond_true_correct tge tf cond args lbl x rs0 m' tc).
  intros [[rs' [jmp [EX [GOTO INV]]]] | [jmp [JMP GOTO]]]; auto.
  Local Transparent destroyed_by_cond.
  + (* reg conditional *)
    exploit exec_straight_steps_2; eauto.
    intros [ofs' [PC2 CT2]].
    exploit find_label_goto_label; eauto.
    intros [tc' [rs1 [GOTO1 [AT' INV1]]]].
    exists (State rs1 m'); split.
    eapply plus_right'. eapply exec_straight_steps_1; eauto.
    econstructor; eauto.
    eapply find_instr_tail; eauto.
    rewrite GOTO. eexact GOTO1.
    traceEq.
    econstructor; eauto.
    apply agree_exten with rs'; auto with asmgen.
    apply agree_undef_regs with rs0; auto with asmgen.
    congruence.
  + (* imm conditional *)
    exploit find_label_goto_label; eauto.
    intros [tc' [rs' [GOTO1 [AT' INV1]]]].
    exists (State rs' m'); split.
    apply plus_one. econstructor; eauto.
    eapply find_instr_tail. rewrite JMP in H6; eauto.
    rewrite GOTO. eexact GOTO1.
    econstructor; eauto.
    apply agree_undef_regs with rs0; auto with asmgen.
    congruence.
...
- (* Mcond false *)
  exploit eval_condition_lessdef. eapply preg_vals; eauto. eauto. eauto. intros EC.
  left; eapply exec_straight_steps; eauto. intros. simpl in TR.
  destruct (transl_cond_false_correct tge tf cond args lbl _ rs0 m' _ TR EC) as [rs' [A B]].
...
- (* Mjumptable *)
  ...
  instantiate (2 := rs0#X5 <- Vundef #X6 <- Vundef).
  Simpl. eauto.
  eauto.
  intros [tc' [rs' [A [B C]]]].
  ...
  simpl. intros. rewrite C; auto with asmgen.
  inv H8. rewrite Pregmap.gso; auto. rewrite Pregmap.gso; auto.
  congruence.
...
- (* Mreturn *)
  ...
  destruct (X (Pj_r RA (Mach.fn_sig f) :: x)) as [rs2 [P [Q [R S]]]].
  exploit exec_straight_steps_2. eexact P. eauto. eauto. eapply functions_transl; eauto. eauto.
  intros [ofs' [Y Z]].
  left; econstructor; split.
  eapply plus_right'. eapply exec_straight_exec; eauto.
...
- (* internal function *)
  ...
  set (rs2 := nextinstr (rs0#X30 <- (parent_sp s) #SP <- (Vptr stk Int.zero))).
  ...
  simpl. auto.
  simpl. unfold exec_store. change (rs2 RA) with (rs0 RA).
