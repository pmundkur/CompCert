...
Proof.
  ...
  - subst. TrivialExists.
    econstructor. EvalOp. intuition.
    econstructor. eauto. constructor. simpl. decEq.
    rewrite Val.add_assoc.
    pattern (Val.add (Val.add v1 (Vint n1)) (Val.add sp (Vint n2))).
    rewrite Val.add_permut.
    replace (Val.add (Vint (Int.add n1 n2)) v1)
       with (Val.add (Val.add v1 (Vint n1)) (Vint n2)). reflexivity.
    rewrite Val.add_commut.
    rewrite Val.add_permut.
    rewrite Val.add_commut.
    replace (Vint (Int.add n1 n2)) with (Val.add (Vint n2) (Vint n1)). reflexivity.
    rewrite Int.add_commut. reflexivity.
  - subst. TrivialExists.
    econstructor. EvalOp. intuition.
    econstructor. eauto. constructor. simpl. decEq.
    rewrite Val.add_permut. rewrite Val.add_commut.
    replace (Val.add (Val.add sp (Vint n1)) (Vint n2))
       with (Val.add sp (Vint (Int.add n1 n2))). reflexivity.
    rewrite Val.add_assoc.
    replace (Val.add (Vint n1) (Vint n2)) with (Vint (Int.add n1 n2)); reflexivity.
...
Proof.
  ...
  predSpec Int.eq Int.eq_spec n Int.zero.
  - subst n. exists x; split; try assumption.
  ...
  - destruct x; simpl in H0; try discriminate.
    destruct (Int.ltu n (Int.repr 31)) eqn:NLTU; inv H0.
    assert (NZ: Int.unsigned n <> 0).
    { intro EQ. apply f_equal with (f := Int.repr) (B := int) in EQ.
      rewrite Int.repr_unsigned in EQ. fold Int.zero in EQ. contradiction. }
    assert (LT: Int.unsigned n < 31).
    { unfold Int.ltu in NLTU.
      case (zlt (Int.unsigned n) (Int.unsigned (Int.repr 31))); intros.
      - rewrite Int.unsigned_repr in l; auto.
        unfold Int.max_unsigned; simpl; omega.
      - rewrite zlt_false in NLTU. inversion NLTU. auto. }
    assert (X: eval_expr ge sp e m le
               (Eop (Oshrimm (Int.repr (Int.zwordsize - 1))) (a ::: Enil))
               (Vint (Int.shr i (Int.repr (Int.zwordsize - 1))))).
    { exploit eval_shrimm. eexact H. instantiate (1 := (Int.repr (Int.zwordsize - 1))).
      intros [v [E LD]]. EvalOp. }
    ...
    { unfold shrximm_inner. EvalOp. simpl. f_equal.
      unfold Int.ltu. rewrite zlt_true; auto.
      unfold Int.iwordsize, Int.sub.
      repeat (rewrite Int.unsigned_repr). apply Int.ltu_inv in NLTU.
      - omega.
      - unfold Int.zwordsize, Int.max_unsigned; simpl. omega.
      - replace Int.zwordsize with 32 by (compute; auto).
        generalize (Int.unsigned_range n); intros. unfold Int.max_unsigned.
        assert (Bound1: 32 - Int.unsigned n < 32) by omega.
        assert (Bound2: 32 <= Int.modulus -1) by (simpl; omega).
        omega.
      - unfold Int.zwordsize, Int.max_unsigned; simpl. omega. }
    eexists; split. EvalOp. econstructor. EvalOp; simpl; f_equal.
    econstructor. simpl; f_equal.
    unfold Int.ltu; rewrite zlt_true.
    rewrite Int.shrx_shr_2; auto.
...
Proof.
  ...
  - unfold cast8signed_lower.
      assert (SHL: eval_expr ge sp e m le (Eop (Oshlimm (Int.repr 24)) (e0 ::: Enil)) (Val.shl x (Vint (Int.repr 24)))).
      EvalOp.
    exists (Val.shr (Val.shl x (Vint (Int.repr 24))) (Vint (Int.repr 24))).
    split.
    + EvalOp.
    + destruct x; simpl; auto.

      assert (WS: Int.ltu (Int.repr 24) Int.iwordsize = true).
      auto.

      apply Val.lessdef_same.
      rewrite WS; simpl.
      apply f_equal.
      apply Int.sign_ext_shr_shl; compute; auto.
Qed.
...
Proof.
  ...
  - InvEval. TrivialExists.
  - unfold cast16signed_lower.
      assert (SHL: eval_expr ge sp e m le (Eop (Oshlimm (Int.repr 16)) (e0 ::: Enil)) (Val.shl x (Vint (Int.repr 16)))).
      EvalOp.
    exists (Val.shr (Val.shl x (Vint (Int.repr 16))) (Vint (Int.repr 16))).
    split.
    + EvalOp.
    + destruct x; simpl; auto.

      assert (WS: Int.ltu (Int.repr 16) Int.iwordsize = true).
      auto.
